...
export function buildCutPlanMarkdown (planks: Plank[], required: RequiredPieceInput[], unit: 'mm' | 'inch'): string {
  ...
  for (const [idx, plank] of sortedPlanks.entries()) {
    let plankHeader = `Plank ${idx + 1}: ${fmtPiece(plank.lengthMm, plank.widthMm, unit)}`
    if (plank.isHalved) {
      plankHeader += ' (halved)'
    }
...
export function buildFullMarkdown (
  planks: Plank[],
  required: RequiredPieceInput[],
  purchaseItems: PurchasePlanItem[],
  unit: 'mm' | 'inch',
  currencySymbol: string,
): string {
  const parts: string[] = []
  parts.push('# Plank Cut Planner Report')
  parts.push('')
  parts.push('Generated by Plank Cut Planner')
  parts.push('')
  // Cut plan section
  parts.push(buildCutPlanMarkdown(planks, required, unit))
  parts.push('')
  // Purchase plan section
  parts.push(buildPurchasePlanMarkdown(purchaseItems, unit, currencySymbol))
  return parts.join('\n')
}
